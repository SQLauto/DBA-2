declare @columnExists bit = 0
	if exists(select 1 from sys.tables t
					...
					where
						sc.name = 'deployment'
					and t.name =  'PatchingPreValidationError'
					and co.Name = 'TestRepresentsQueueDataBeingEmptied')
	begin
		set @columnExists = 1
	end
...
if (@columnExists = 1)
begin
	declare @schemaName varchar(128) = 'deployment'
	declare @tableName varchar(128) = 'PatchingPreValidationError'
	declare @columnName varchar(128) = 'TestRepresentsQueueDataBeingEmptied'
...
if exists (
	...
	exec sp_rename 'deployment.PatchingPreValidationError.TestRepresentsQueueDataBeingEmptied', 'ActiveSystemExpectedFailure', 'COLUMN';
end
