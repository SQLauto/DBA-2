go
	...
	if exists(select 1 from sys.tables t
					...
					where
						sc.name = 'deployment'
					and t.name =  'PatchingScriptsRun'
					and co.Name = 'Id')
	begin
		set @columnExists = 1
	end

	declare @columnIsIdentity bit = 0
	if exists(select 1 from sys.tables t
					inner join sys.schemas sc on 
						t.schema_id = sc.schema_id
					inner join sys.columns co on
	...
	begin
		set @columnIsIdentity = 1
	end
	
	if (@columnExists = 1 and @columnIsIdentity = 0)
	begin
			declare @schemaName varchar(128) = 'deployment'
	declare @tableName varchar(128) = 'PatchingScriptsRun'
...
if exists (
	...
	begin
		...
		alter table deployment.PatchingScriptsRun
			drop column Id
	end
	...
	set @primaryKeyName = (select distinct
								...
								and t.name = 'PatchingScriptsRun'
								and kc.type = 'PK')
	if (@primaryKeyName is null and @columnExists = 1)
	begin
		alter table deployment.PatchingScriptsRun
		add constraint pk_PatchingScriptRun primary key (Id)
	end
go