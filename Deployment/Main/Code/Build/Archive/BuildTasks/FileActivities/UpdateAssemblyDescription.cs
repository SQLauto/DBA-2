...
namespace CustomBuildActivities.FileActivities
{
    ...
    public sealed class UpdateAssemblyDescription : CodeActivity
    {
        [RequiredArgument]
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            foreach (var file in Directory.EnumerateFiles(sourcesDirectory, assemblyInfoFileMask, SearchOption.AllDirectories))
            {
                ...
                foreach (var attribute in new[] { "AssemblyDescription"})
                {
                    var regex = new Regex(attribute + @"\("".*""\)");
                    var match = regex.Match(text);
                    if (!match.Success) continue;
                    text = regex.Replace(text, attribute + "(\"" + context.GetValue(AssemblyDescription) + "\")");
