...
namespace LabManager
{
    ...
    public sealed class DoesRigExist : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            try
            {
                ...
                Configuration[] configurations = soapClient.ListConfigurations(auth, 1);
                foreach (Configuration configuration in configurations)
                {
                    if (configuration.name == context.GetValue(this.RigName))
                        verified = true;
                }
            }
            catch(Exception ex)
