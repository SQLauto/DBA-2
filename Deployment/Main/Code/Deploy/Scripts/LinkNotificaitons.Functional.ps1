...
.SWITCH IsLocal
	Used when running on Dev Machines to create a net use connection.
#>
[cmdletbinding()]
param
(
...
param()
	...
	Write-Host "Updating SendMail config"
	$serverIP = $machines.Item("TS-APP1")
	$cisIP = $machines.Item("TSCIS1")
	...
	$configInfo.Content | Update-ConfigValue -ConfigSection "appSettings" -AppSettingKey "imageUrlLocation" -Value "http://$cisIP/content/images/email/" | Out-Null
