...
if exists(select 1 from msdb.dbo.sysjobs where name=N'DB Maint - DBA STATS Weekend')
begin
	goto EndSave
end
...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[FTP Notifications]]    Script Date: 19/10/2015 11:16:48 ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DB Maint - DBA STATS Weekend', 
		@enabled=0, 
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [UpDateSTATS]    Script Date: 19/10/2015 11:16:48 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'OLA STATS Weekends', 
		@enabled=0, 
		@freq_type=8, 
		@freq_interval=64, 
		@freq_subday_type=1, 
		...
		@schedule_uid=N'f558e90c-f4d3-4824-b956-db4031da9ac2'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
