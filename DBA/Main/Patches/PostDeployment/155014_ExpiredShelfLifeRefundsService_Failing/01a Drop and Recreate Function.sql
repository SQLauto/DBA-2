/*	
	Recreates the List_To_Table function, allowing NULL values in the return TABLE type.
	Fixes the failing ExpiredShelfLifeRefundsService that runs on the RefundManager db 
*/
...
--BEGIN
	CREATE FUNCTION [Refunds].[List_to_Table] (@list nvarchar(MAX), @delimiter NCHAR(1))
		RETURNS @tbl TABLE (value NVARCHAR(50) NULL) 
	AS
	BEGIN
		DECLARE @pos        INT,
				@nextpos    INT,
