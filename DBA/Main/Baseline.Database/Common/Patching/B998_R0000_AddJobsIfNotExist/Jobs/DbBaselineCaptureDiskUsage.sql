...
EXEC #SqlJobExists @JobName = 'DB $(databasename) - Capture Disk Usage', @jobExists  = @jobExists  OUTPUT;
IF @jobExists = 0
BEGIN 
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DB $(databasename) - Capture Disk Usage', 
			@enabled=0, 
	...
	QuitCreateWithRollback:
		...
		BEGIN
			...
			RAISERROR('JOB CREATION Failure for DB $(databasename) - Capture Disk Usage',16,1)
		END
	CreateSave:
...
WHILE EXISTS(select JS.step_id from msdb.dbo.sysjobs J
				...
				where name ='DB $(databasename) - Capture Disk Usage')
BEGIN
	select @step_id=max(step_id) from msdb.dbo.sysjobs J
	JOIN msdb.dbo.sysjobsteps JS ON J.job_id=JS.job_id
	where name ='DB $(databasename) - Capture Disk Usage'

	EXEC msdb.dbo.sp_delete_jobstep @job_name=N'DB $(databasename) - Capture Disk Usage', @step_id=1
END

EXEC msdb.dbo.sp_update_job @job_name='DB $(databasename) - Capture Disk Usage',@enabled = 0

/* STEP 1*/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_name=N'DB $(databasename) - Capture Disk Usage', @step_name=N'Capture Disk Usage', 
		@step_id=1, 
		...
		@command=N'EXEC [capture].[DiskUsage]', 
...
where J.name = 'DB $(databasename) - Capture Disk Usage'
...
EXEC msdb.dbo.sp_add_jobschedule @job_id=@job_id, @name=N'DB $(databasename) - Capture Disk Usage Once Per Day', 
		@enabled=1, 
		@freq_type=4,
		@freq_interval=1,		
		@freq_subday_type=1, 
