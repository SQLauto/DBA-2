...
EXEC #SqlJobExists @JobName = 'DB $(databasename) - Capture CPU Utilisation', @jobExists  = @jobExists  OUTPUT;
IF @jobExists = 0
BEGIN 
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DB $(databasename) - Capture CPU Utilisation', 
			@enabled=0, 
	...
	QuitCreateWithRollback:
		...
		BEGIN
			...
			RAISERROR('JOB CREATION Failure for DB $(databasename) - Capture CPU Utilisation',16,1)
		END
	CreateSave:
...
WHILE EXISTS(select JS.step_id from msdb.dbo.sysjobs J
				...
				where name ='DB $(databasename) - Capture CPU Utilisation')
BEGIN
	select @step_id=max(step_id) from msdb.dbo.sysjobs J
	JOIN msdb.dbo.sysjobsteps JS ON J.job_id=JS.job_id
	where name ='DB $(databasename) - Capture CPU Utilisation'

	EXEC msdb.dbo.sp_delete_jobstep @job_name=N'DB $(databasename) - Capture CPU Utilisation', @step_id=1
END

EXEC msdb.dbo.sp_update_job @job_name='DB $(databasename) - Capture CPU Utilisation',@enabled = 0

/* STEP 1*/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_name=N'DB $(databasename) - Capture CPU Utilisation', @step_name=N'Capture CPU Utilisation', 
		@step_id=1, 
		...
		@command=N'EXEC Capture.CpuUsage', 
...
where J.name = 'DB $(databasename) - Capture CPU Utilisation'
...
EXEC msdb.dbo.sp_add_jobschedule @job_id=@job_id, @name=N'DB $(databasename) - Capture CPU Utilisation - Every 15 minutes', 
		@enabled=1, 
		@freq_type=4, 
		@freq_interval=1, 
		@freq_subday_type=4, 
