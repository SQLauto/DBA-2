EXEC #CreateDummyStoredProcedureIfNotExists 'capture', 'SQLServerLogs'
GO
ALTER procedure [capture].[SQLServerLogs]
AS
BEGIN
	SET NOCOUNT ON;
	declare @DateStart DATETIME, @DateEnd DATETIME
	select @DateStart=datetimecovered from [capture].[SQLServerLogsWindow] where eventname='sqlserverlog'
    
	if(@DateStart is null)
	BEGIN
		SET @DateStart=getdate()-1
	END
	set @DateEnd=getdate()
...
INSERT INTO #ExclusionList VALUES ('Error: %, Severity: %, State: %')
INSERT INTO #ExclusionList VALUES ('option SINGLE_USER to ON')
INSERT INTO #ExclusionList VALUES ('option QUOTED_IDENTIFIER to ON')
INSERT INTO #ExclusionList VALUES ('option RECOVERY to SIMPLE')
INSERT INTO #ExclusionList VALUES ('AppDomain%SSISDB.dbo%created')
INSERT INTO #ExclusionList VALUES ('Unsafe assembly %microsoft.sqlserver.integrationservices.server')
...
INSERT INTO #ExclusionList VALUES ('BACKUP DATABASE successfully processed')
INSERT INTO #ExclusionList VALUES ('BACKUP DATABASE WITH DIFFERENTIAL successfully processed')
...
CREATE TABLE #Info ([LogDate] DATETIME, [ProcessInfo] VARCHAR(1000), [message] VARCHAR(7000))

-- Read error log
INSERT INTO #Info EXEC master.dbo.xp_readerrorlog 0, 1, NULL, NULL, @DateStart, @DateEnd
...
IF @TraceFile <> ''
BEGIN
   INSERT INTO #Info ([LogDate], [ProcessInfo], [message])
   SELECT MIN([ftg].[StartTime]),'TraceFile', '['+ISNULL([ftg].[DatabaseName],'')+'] '+ISNULL([otn].[ObjectName],'')+' '+ISNULL(REPLACE([te].[name],'Object:',''),'')+' by '+ISNULL([ftg].[LoginName],'')+' with: '+ISNULL([ftg].[ApplicationName],'n/a')
...
END
-- Delete exclusions
DELETE [i] FROM #Info [i] INNER JOIN #ExclusionList [li] ON [i].[message] LIKE '%'+[li].[stringValue]+'%'
...
-- Read TempDB size
   ...
   BEGIN
      INSERT INTO #Info ([LogDate],[ProcessInfo],[message])
      VALUES (GETDATE(), 'TempDB','[TempDB] current size is more than twice greater of initial size')
   END


-- Finish
if Exists(select 1 from [capture].[SQLServerLogsWindow] where eventname='sqlserverlog')
BEGIN
	update [capture].[SQLServerLogsWindow] SET datetimecovered=@DateEnd where eventname='sqlserverlog'
END
ELSE
BEGIN
	insert into [capture].[SQLServerLogsWindow]
	(eventname,currenttable,datetimecovered)
	values('sqlserverlog','[capture].[SQLServerLogsData]',@DateEnd)
END

insert into [capture].[SQLServerLogsData]
(
	 [capturedate]
	,[ProcessInfo]
	,[message]
...
SELECT *,@@servername as servername FROM #Info ORDER BY [LogDate] desc
