...
param(
    ...
    [string]$DatabaseName,
    [string]$DropFolder,
    [string] $Environment = "PaymentAssurance",
    [string] $Config = "TestRig.PaymentAssurance",
	[Parameter()]
...
$createFunc = {
	...
	if ($Action -eq 'c') {
		...
		if($retVal -ne 0){
		 	Write-Host2 -Type Failure -Message "Failed to create database $DatabaseName from $DataSource"
		 	return 1
		}

		$retVal = Invoke-LocalDatabaseDeployment @params -BaselineScript (Join-Path $localDbFolder "BaseLine\Baseline.sql")  -Role "$DatabaseName Baseline Schema"
		...
		Write-Host2 -Type Success -Message "Successfully recreated database (in simple recovery): $DatabaseName on $DataSource"
	}
