...
function Write-ObjectToSQL
{
    ...
    (
        ...
        #optional parameter which sets the datatype of table column
    ...
    Begin
    {
        ...
        if (!$tablecreated -and !$ConnectionString) 
        {
            if ( CheckIfTableExistsSQL )
            {
                ...
                $sqltext="truncate table $SchemaName.$TableName"
                #$table=Invoke-Sqlcmd -query $sqltext -ServerInstance $Server -Database $Database
    ...
    Process
    {
        ...
        if (!$tablecreated)
        {
            ...
            foreach($key in $datasample.Keys)
            {
                ...
                if($datatype -eq "System.DBNull")
                {
                    $columntype=($columnsObj | Where-Object{$_.name -eq $key}).datatype
                    if($columntype -match 'varbinary')
                    {
                        $datatype="System.Int64"
                    }
                    if($columntype -match 'varchar')
                    {
                        $datatype="System.String"
                    }
                    elseif($columntype -match 'bigint')
                    {
                        $datatype="System.Int64"
                    }
                    elseif($columntype -match 'int')
                    {
                        $datatype="System.Int32"
                    }
                    elseif($columntype -match 'datetime')
                    {
                        $datatype="System.String"
                    }
                    elseif($columntype -match 'xml')
                    {
                        $datatype="System.String"
                    }
                }		
                ...
                try {
                    ...
                    {
                        if($columnsObj -eq $null)
                        {
                            $columntype=$($numbertypes.($datatype))                            
                        }
                        else
                        {
                        ...
                        $querystring += ", $quoteFirst$prekey$($key.Replace(' ','_'))$quoteLast $columntype"
                        if ($PrimaryKey) {
                    ...
                    elseif ( $stringtypes.ContainsKey( $datatype ) )
                    {
                        ...
                        if($columnsObj -eq $null)
                        {
                            $columntype=$($stringtypes.($datatype))                          
                        }
                        else
                        {
                        ...
                        $querystring += ", $quoteFirst$prekey$($columnname.Replace(' ','_'))$quoteLast $columntype"
                        if ($PrimaryKey) {
                            if ($key -ne $PrimaryKey) {
                                $querystring += " NULL"
                            }
        ...
        foreach($key in $datasample.Keys)
        {
            ...
            elseif($datatype -eq "System.DBNull")
            {
                    $columntype=($columnsObj | Where-Object{$_.name -eq $key}).datatype
                    if($columntype -match 'varchar')
                    {
            ...
            try {
                if ( $numbertypes.ContainsKey( $datatype ) )
                {
                    ...
                    $columnval=$($InputObject.$key)
                    $x=$key
                    if ($columnval -eq $null -or $columnval.GetType().Name -eq 'DBNull')
                    {
                    ...
                    else
                    {
                        ...
                        {
                            $null = $strBuilderValues.Append(", $columnval")
                        }
                ...
                elseif ( $stringtypes.ContainsKey( $datatype ) )
                {
                    ...
                    $null = $strBuilderColumns.Append(", $quoteFirst$prekey$($columnname)$quoteLast")
                    $strtmp = $InputObject.$key -replace "'", "''"
                    if( $datatype -eq "System.DateTime")
                    {
                       $columnval=[regex]::Replace(([datetime]::ParseExact($strtmp,"dd/MM/yyyy HH:mm:ss",$null)).ToString(),"(\d+)/(\d+)/(\d+)(.+)",'$3/$2/$1$4') -replace "/","-"
                    }
                    else
                    {
                        $columnval=$strtmp
                    ...
                    {
                        $null = $strBuilderValues.Append(", N'$columnval'")
                    }
                }
                else
                {
